...
export default async function handler(request, response) {
    ...
    const prompt = `Gere UMA única questão aleatória de múltipla escolha sobre o assunto "${assunto}", dentro da matéria de "${materia}", no estilo de prova "${estilo}" e com nível de dificuldade "${dificuldade}". A resposta DEVE ser um objeto JSON válido, e NADA MAIS além do JSON. A estrutura do JSON deve ser exatamente a seguinte: { "question": "o enunciado completo da pergunta", "options": ["alternativa 1", "alternativa 2", "alternativa 3", "alternativa 4"], "answer": 0, "explanation": "uma explicação detalhada e clara da resposta correta." }. O campo "answer" deve ser o índice (de 0 a 3) da alternativa correta no array "options".`;
